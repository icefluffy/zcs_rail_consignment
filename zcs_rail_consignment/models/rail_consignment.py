...
class RailConsignmentValidator(models.Model):
    ...
    def _decode_evn(self, evn):
        ...
        # decode all wagons (type_code 0x-9x)
        if (0 <= type_code <= 49) or (80 <= type_code <= 89):
            wagon_class_digit = evn[4]
            digits_6_to_8 = evn[5:8]

            wagon_class_desc = WAGON_CLASS.get(
                wagon_class_digit,
                f"Unknown wagon class {wagon_class_digit}"
            )

            subtype_map = WAGON_SUBTYPE.get(wagon_class_digit, {})
            subtype_desc = subtype_map.get(
                digits_6_to_8,
                f"Sub-type {digits_6_to_8} (not yet decoded)"
            )

            technical_desc = f"{wagon_class_desc} | {subtype_desc}"

        elif 50 <= type_code <= 79:
            technical_desc = f"Passenger vehicle technical code {technical_code}"

        elif 90 <= type_code <= 99:
            technical_desc = f"Tractive stock class {technical_code}"

        else:
            technical_desc = f"Technical code {technical_code}"
